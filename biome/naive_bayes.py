...
class GNB():
    ...
    def get_GNB(self):
        ...
        # Check data type:
        if isinstance(self.x_train, np.ndarray) is False or \
                isinstance(self.y_train, np.ndarray) is False:
            raise TypeError("Training data must be numpy arrays.")
        # Check shape:
        x_shape = np.shape(self.x_train)
        y_shape = np.shape(self.y_train)
        ...
        trained_model = GaussianNB().fit(self.x_train, self.y_train)
